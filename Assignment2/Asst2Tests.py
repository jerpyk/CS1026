...
def runTest(file, inText):
    ...
    if file == "pizzaReceipt":
        res = subprocess.run("python -c \"from pizzaReceipt import *; generateReceipt([" + inText + "])\"",
...
if studentOutput.find("\"X\"\n('") != -1 and studentOutput.count("Choose a size:") == 2 and studentOutput.count(
        "Type in one of our toppings") == 7 and studentOutput.find("1.49") != -1 and studentOutput.find("12.98") != -1:
...
studentOutput = runTest("order",
                        "y\nm\nsausage\nbacon\nonion\nX\ny\nXl\nchicken\ntomato\nspinach\nmushroom\nx\ny\nm\nolive\nbroccoli\nhot pepper\ngreen pepper\nx\nno\n")

if studentOutput.count("Type in one of our toppings") == 14 and studentOutput.find("4.68") != -1 and studentOutput.find(
        "40.65") != -1:
